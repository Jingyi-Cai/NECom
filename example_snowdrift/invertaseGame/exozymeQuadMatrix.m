function [gr, vErange, combinations, info] = exozymeQuadMatrix(X, epsCap, costExo, k, r, vUub, qc, nStep, params)
...
for j = 1:N
    LP.quadcon(j).Qrow = [v.vB(j); v.vB(j); v.vE(j); v.vE(j)];
    LP.quadcon(j).Qcol = [v.vB(j); v.vE(j); v.vB(j); v.vE(j)];
    LP.quadcon(j).Qval = (r / vUub / X(j)) * [1; qc; qc; qc ^ 2];
    LP.quadcon(j).q = sparse(v.vM(j), 1, -1, nV,1);
    LP.quadcon(j).rhs = 0;
    LP.quadcon(j).sense = '<';
    LP.quadcon(j).name = ['vMlb_' num2str(j)];
end
...
if isscalar(nStep)
    ...
    vEmax = sol.objval;
    vErange = linspace(0, vEmax, nStep)';
else
    vErange = nStep;
    nStep = numel(vErange);
end
...
nComb = nStep ^ N;
combinations = permn((1:nStep)', N);
gr = zeros(nComb, N);
LP.obj(:) = 0;
LP.obj(v.vB) = 1./X;
for j = 1:nComb
    [LP.lb(v.vE), LP.ub(v.vE)] = deal(vErange(combinations(j, :)));
    sol = gurobi(LP, params);
