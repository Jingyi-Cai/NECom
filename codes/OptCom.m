function [x,exitflag,index,Lmodel,fluxes,info]=OptCom(themodel,parameters,X)
%{ 
...
numex=length( parameters.spsInds);   
%-----------variables in dual problem----------------------------------------------------
...
lhs=zeros(numex,1);
sparse1=sparse(parameters.spsInds,1:parameters.numSub_ExRxn,ones(parameters.numSub_ExRxn,1),numex,parameters.numSub_ExRxn);
...
fun = @(x) x(themodel.spBm(1))+x(themodel.spBm(2));
